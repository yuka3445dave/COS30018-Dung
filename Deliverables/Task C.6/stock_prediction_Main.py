...
def ensemble_weighted(pred_dl, pred_sarima, w_dl = 0.2):
    """
    Weighted ensemble:
...
def choose_weight_by_val_mae(y_val, pred_dl_val, pred_sarima_val):
    """
    Pick w that minimizes validation MAE among a small grid.
    """
    best_w, best_mae = None, float("inf")
    for w in [0.2, 0.4, 0.5, 0.6, 0.8]:
        pred = ensemble_weighted(pred_dl_val, pred_sarima_val, w_dl=w)
        mae, _, _ = regression_metrics(y_val, pred)
        if mae < best_mae:
            best_mae = mae
            best_w = w
...
main_model = build_recurrent_model(
...
# Build train/test Close in real price units for SARIMA
first_test_date = test_dates[0]
first_test_pos = df_clean.index.get_loc(first_test_date)
train_close = df_clean[TARGET_COLUMN].iloc[:first_test_pos]
test_close = df_clean[TARGET_COLUMN].iloc[first_test_pos:first_test_pos + len(test_dates)]\

RUN_TASK6 = True 
if RUN_TASK6:

    TASK5_MODE = "multivariate"
    FORECAST_HORIZON = 1
    K_STEPS = 1

    TASK6_EXPERIMENTS = [
        # Required baseline: SARIMA + LSTM
        {"dl_type": "LSTM", "epochs": 25, "batch": 64, "order": (1,1,1), "seasonal": (1,0,1,5), "w": 0.5},

        # Different DL network
        {"dl_type": "GRU",  "epochs": 25, "batch": 64, "order": (1,1,1), "seasonal": (1,0,1,5), "w": 0.5},

        # Different SARIMA hyperparameters
        {"dl_type": "LSTM", "epochs": 25, "batch": 64, "order": (2,1,2), "seasonal": (1,0,1,5), "w": 0.5},

        # Another model family (optional but nice): RNN
        {"dl_type": "RNN",  "epochs": 25, "batch": 64, "order": (1,1,1), "seasonal": (1,0,1,5), "w": 0.5},

        {"dl_type": "GRU", "epochs": 25, "batch": 64, "order": (1,1,1), "seasonal": (1,0,1,5), "w": 0.8},
        
        {"dl_type": "GRU", "epochs": 25, "batch": 64, "order": (1,1,1), "seasonal": (1,0,1,5), "w": 0.2},
    ]
    ...
    for cfg in TASK6_EXPERIMENTS:
        ...
        dl_model = build_recurrent_model(
            layer_name=cfg["dl_type"],
            input_shape=input_shape,
            num_layers=NUM_LAYERS,
            units=UNITS,
            dropout=DROPOUT,
            dense_units=1,  # single-step for C.6
            optimizer="adam",
            loss="mean_squared_error",
        )
        ...
        y_train_step = y_train[:, 0] if y_train.ndim == 2 else y_train   # take t+1 only
        y_test_step  = y_test[:, 0]  if y_test.ndim == 2 else y_test

        t0 = time.perf_counter()
        dl_model.fit(x_train, y_train_step, epochs=cfg["epochs"], batch_size=cfg["batch"], verbose=0)
        dl_train_time = time.perf_counter() - t0

        dl_pred_scaled = dl_model.predict(x_test, verbose=0)
        dl_pred = scalers["target_scaler"].inverse_transform(dl_pred_scaled).reshape(-1)

        # Actual values
        y_true = scalers["target_scaler"].inverse_transform(y_test_step.reshape(-1, 1)).reshape(-1)

        dl_mae, dl_mse, dl_rmse = regression_metrics(y_true, dl_pred)

        # --- Fit SARIMA & forecast ---
        t1 = time.perf_counter()
        sarima_fitted = fit_sarima(train_close, order=cfg["order"], seasonal_order=cfg["seasonal"])
        sarima_fit_time = time.perf_counter() - t1

        sarima_pred = forecast_sarima(sarima_fitted, steps=len(y_true)).to_numpy()
        sa_mae, sa_mse, sa_rmse = regression_metrics(y_true, sarima_pred)

        # --- Ensemble ---
        ens_pred = ensemble_weighted(dl_pred, sarima_pred, w_dl=cfg["w"])
        ...
        results.append({
            ...
            "epochs": cfg["epochs"],
            "batch": cfg["batch"],
            "SARIMA_order": cfg["order"],
            "SARIMA_seasonal": cfg["seasonal"],
            "w_dl": cfg["w"],
            "DL_MAE": dl_mae,
            "SARIMA_MAE": sa_mae,
            "ENS_MAE": en_mae,
    ...
    results_df = pd.DataFrame(results).sort_values("ENS_MAE")
    print(results_df)
...
t0 = time.perf_counter()
history = main_model.fit(x_train, y_train, epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=1)
...
# -----------------------------
predicted_scaled = main_model.predict(x_test)
...
next_scaled = main_model.predict(real_data)
